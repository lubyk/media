...
effect.fragment = four.Effect.Shader [[
  ...
  void main() {
    ...
    vec4 foo = texture(textfoo, v_tex + 0.05*vec2(v_vertex.x*sin(t), v_vertex.y * sin(t/2)));

    float bw = (foo.r+foo.g+foo.b)/3;
    float r = foo.r * sin(v.x/30) * sin(v.y * bw); //sin(v.x + bw * t); // foo.r;
    float g = foo.g * sin(v.x/10.4) * sin(v.y * bw); //sin(v.x + bw * t); // foo.g;
    float b = foo.b * sin(v.x) * sin(v.y * bw); //sin(v.x + bw * t); // foo.b;
    // color = foo;
    color = vec4(r, g, b, 1);
    // vec4 v = 20 * (v_vertex + vec4(sin(t/10), sin(t/20), sin(t/30), 0.0));
    // float r = sat + 0.4 * (sin(v.x) + sin(v.y));
    // float g = sat + 0.4 * (sin(v.x) + sin(v.y));
    // float b = sat + 0.4 * (sin(v.x) + sin(v.y));
    // // blend effect with interpolated color
    // color = vec4(foo.r * (v_color.r + r), foo.g * (v_color.g + g), foo.b * (v_color.b + b), 1);
    // color = vec4(v_color.r, v_color.g, v_color.b, 1);
    // color = vec4(foo.r, foo.g, foo.b, 1);
  }
]]
